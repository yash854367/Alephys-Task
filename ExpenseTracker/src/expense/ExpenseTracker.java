...
public class ExpenseTracker {
    ...
    private static void viewMonthlySummary() {
        ...
        for (Transaction t : transactions) {
            String ttype = t.type.replaceAll("[^A-Za-z]", "").trim().toUpperCase();
            if (YearMonth.from(t.date).equals(yearMonth)) {
                if (ttype.equals("I")) {
                    ...
                    System.out.println("Warning: Unknown type: [" + t.type + "]");
                }
            }
        }
    ...
    static class Transaction {
        ...
        Transaction(String type, String category, double amount, LocalDate date, String description) {
            ...
            this.type = type.replaceAll("[^A-Za-z]", "").trim().toUpperCase();
        ...
        static Transaction fromFileString(String line) {
            try {
                ...
                String type = parts[0].replaceAll("[^A-Za-z]", "").trim().toUpperCase();
